...
function Profile() {
  ...
  const fetchData = async () => {
    const endpointURL = 'https://script.google.com/macros/s/AKfycbzFKN07XS2IozAXsaZzCugG72jh2NSwfou9fKAfEV3FOiYBHS13qkb_QlWq6ZC7OqL3/exec';
    setIsLoading(true);
    try {
  ...
  const handleCheckboxChange = (uniqueKey, isSelected) => {
    const price = rewards.find(reward => reward.uniqueKey === uniqueKey).innocreditPrice;
    ...
    setSelectedRewards(prevSelectedRewards => {
      ...
      setTentativePoints(currentPoints => {
        const selectedItems = rewards.filter(reward => newSelectedRewards[reward.uniqueKey]);
        const totalCost = selectedItems.reduce((acc, item) => acc + item.innocreditPrice, 0);
  ...
  const handlePurchase = () => {
    ...
    const selectedItems = rewards.filter(reward => selectedRewards[reward.uniqueKey]);
    const totalCost = selectedItems.reduce((acc, item) => acc + item.innocreditPrice, 0);
  ...
  const finalizePurchase = async () => {
    ...
    if (tentativePoints >= totalCost) {
      ...
      try {
        ...
        const response = await axios.post('https://script.google.com/macros/s/AKfycbzFKN07XS2IozAXsaZzCugG72jh2NSwfou9fKAfEV3FOiYBHS13qkb_QlWq6ZC7OqL3/exec', payload);
      ...
      <div className="purchase-summary-modal">
        ...
        <ul>
          {summaryItems.map(item => (
            <li key={item.uniqueKey}>{item.itemName} - {item.innocreditPrice} points</li>
          ))}
        </ul>
        <div className="summary-total">Total: {summaryItems.reduce((acc, item) => acc + item.innocreditPrice, 0)} points</div>
             ...
             <MDBTableBody>
                {rewards.map((reward) => (
                  <tr key={reward.uniqueKey}>
                    <td>
                      <img 
                        src={reward.image.preview_url || reward.image.download_url} 
                        alt={reward.itemName}
                    ...
                    <td>
                      <input
                        ...
                        onChange={(e) => handleCheckboxChange(reward.uniqueKey, e.target.checked)}
                      />
                    </td>
                  </tr>
                ))}
